...
// ===== CONFIGURAZIONE WEB3MODAL (SOLANA PURA) =====
...
const modal = createWeb3Modal({
  ...
  projectId,
  chains: ['solana:mainnet'], // Forza solo Solana (niente ETH/MetaMask)
  themeMode: 'dark',
  themeVariables: {
    '--w3m-z-index': '10000',
...
// ===== CONNECT WALLET (apre il modale) =====
window.connectWallet = async function() {
  try {
    await modal.open();
  } catch (e) {
    console.error('❌ Errore apertura modale:', e);
    window.location.href = 'wallet.html'; // fallback sicuro
  }
};
...
modal.subscribeEvents((event) => {
  if (event.type === 'connect') {
    ...
    window.location.href = 'create.html'; // reindirizza al creatore di token
  }
...
// ===== PULSANTI DI SUPPORTO (per wallet.html) =====
...
console.log('✅ main.js caricato (Web3Modal Solana Puro)');
